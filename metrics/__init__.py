...
from metrics.Columnar_similarity_table import plot_similarity_histogram
#绘图
...
def select(metricname, opt=None):
    if 'e_recall' in metricname:
        # print("AAAAAAAAAAAAA",metricname,metricname.split('@')[-1])
    ...
    elif 'codingrate' in metricname:
        mode = metricname.split('@')[-1]
        return coding_rate.Metric(mode=mode, num_classes=opt.n_classes)
    else:
...
class MetricComputer():
    ...
    def compute_standard(self, opt, model, dataloader, evaltypes, device, mode,
        ...
        with torch.no_grad():
            ...
            for idx, inp in enumerate(final_iter):
                ...
                with context:
                    ...
                    out_dict = compiled_model(input_img.to(device))
        ...
        for evaltype in evaltypes:
            ...
            features = np.vstack(feature_colls[evaltype]).astype('float32') # len(features) test data num
