...
# from utilities.emb_plot import plot_features_on_sphere

from metrics.proxy_heatmap import plot_similarity_matrix
...
dataloaders['evaluation'] = torch.utils.data.DataLoader(
    ...
    batch_size=opt.bs,
    shuffle=False)
# print('len_test_img:',len(datasets['testing'].image_list))
dataloaders['testing'] = torch.utils.data.DataLoader(datasets['testing'],
                                                     num_workers=opt.kernels,
                                                     batch_size=opt.bs,
                                                     shuffle=False)
if opt.use_tv_split:   # Default: false
...
for epoch in epochs:
    ...
    for i, out in enumerate(data_iterator):
        ...
        with context:
            ...
            out_dict = compiled_model(**model_args)
            ...
            loss_args['wmup'] = opt.warmup
            if opt.loss == 'antico':
                loss, proxy_coding_rate, proxies = criterion(**loss_args)
            else:
        ...
        if opt.loss == 'antico':
            proxy_coding_rate_collect.append(proxy_coding_rate.item())
    ...
    if opt.loss == 'antico':
        ac_proxy_result_metrics = {'proxy_coding_rate': np.mean(proxy_coding_rate_collect),
                               }
    ...
    if opt.loss == 'antico':
        for metricname, metricval in ac_proxy_result_metrics.items():
            LOG.progress_saver['Train'].log(metricname, metricval, 'antico')
