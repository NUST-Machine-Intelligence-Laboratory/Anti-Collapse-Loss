...
class Criterion(torch.nn.Module):
    def __init__(self, opt, **kwargs):
        ...
        self.proxies = torch.randn(self.num_proxies, self.embed_dim)
    ...
    def forward(self, batch, labels, avg_batch_features, **kwargs):
        ...
        proxy_align_loss = self.panc(batch, self.proxies, labels)   ## CLS_NUM x DIM
        # print('pro:',self.proxies.shape)
        proxies = torch.nn.functional.normalize(self.proxies, dim=-1)  ## CLS_NUM x DIM
        # print('pr1:', sum((proxies[50])**2))
        proxies = proxies[labels]  ## BS x DIM
        # print('pr2:', labels,sum((proxies[1])**2))
        # print('len_prox:',len(proxies))
        ...
        # print("cond:",cond[1].shape,len(cond))
        if self.noise:
        ...
        # print('nf_latent:',(0.5 * torch.sum(nf_latent**2, dim=(1, ))).shape,'ljd:',log_jac_det.shape)

        loss = mul * (0.5 * torch.sum(nf_latent**2, dim=(1, )) - log_jac_det)
        loss = torch.mean(torch.exp(loss)) + self.w_align * proxy_align_loss
        # m_e_loss = torch.mean(torch.exp(loss))
        # loss = m_e_loss + self.w_align * proxy_align_loss
        return loss
    ...
    def panc(self, batch, proxies, labels, dim=0):
        ...
        # print('sl:',same_labels,same_labels.shape)
        # print('dl:',diff_labels,diff_labels.shape)
        # print('pos:',batch.mm(proxies[u_labels].T),batch.mm(proxies[u_labels].T).shape)
        # print('neg:',batch.mm(proxies.T),batch.mm(proxies.T).shape)
    ...
    def masked_logsumexp(sims, dim=0, mask=None):
        ...
        # print('simsI',sims,sims.shape)
        if mask is not None:
            sims = sims.masked_fill(~mask, torch.finfo(sims.dtype).min)
            # print('mask',mask)
            # print('simsO',sims,sims.shape)
